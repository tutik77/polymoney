...
async def bulk_upsert_markets(
    session: AsyncSession, norms: List[Dict[str, Any]]
) -> Dict[str, int]:
    market_ids = {
        str(n.get("market_external_id")) for n in norms if n.get("market_external_id")
    }
    if not market_ids:
        return {}
...
async def bulk_insert_closed_positions(
    ...
    for n in norms:
        mid = (
            str(n.get("market_external_id"))
            if n.get("market_external_id") is not None
            else None
        )
    ...
    for i in range(0, len(rows), settings.insert_batch_size):
        ...
        stmt = (
            pg_insert(ClosedPosition)
            .values(chunk)
            .on_conflict_do_nothing(constraint="uq_positions_closed_dedupe")
        )
        await session.execute(stmt)
...
async def bulk_upsert_active_positions(
    session: AsyncSession, user: User, norms: List[Dict[str, Any]]
) -> int:
    if not norms:
        return 0
    ...
    for n in norms:
        ...
        payload: Dict[str, Any] = {
            k: v for k, v in n.items() if k in ActivePosition.__table__.columns
        }
    ...
    for i in range(0, len(rows), settings.insert_batch_size):
        ...
        updatable_cols = [
            c.name
            for c in ActivePosition.__table__.columns
            if c.name not in {"id", "user_pk", "asset"}
        ]
        update_dict = {
            col: getattr(insert_stmt.excluded, col) for col in updatable_cols
        }
...
async def get_closed_positions_for_user(
...
async def get_active_positions_for_user(
    session: AsyncSession, user_pk: int
) -> List[ActivePosition]:
    from sqlalchemy import select
    ...
    return (
        (
            await session.execute(
                ...
                .where(ActivePosition.user_pk == user_pk)
                .order_by(ActivePosition.updated_at.desc())
            )
        )
        .scalars()
        .all()
