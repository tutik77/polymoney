...
class User(Base):
    ...
    closed_positions_count: Mapped[int] = mapped_column(
...
class ActivePosition(Base):
    ...
    initial_value: Mapped[Optional[float]] = mapped_column(
        Numeric(38, 8), nullable=True
    )
...
class SimPortfolio(Base):
    ...
    leader_address: Mapped[Optional[str]] = mapped_column(
    ...
    __table_args__ = (
        UniqueConstraint(
            "sim_user", "leader_address", name="uq_sim_portfolio_user_leader"
        ),
    )
...
class SimActivePosition(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            ...
            "leader_address",
            "asset",
            name="uq_sim_active_user_leader_asset",
        ),
    )
...
class SimTrade(Base):
    ...
    title: Mapped[Optional[str]] = mapped_column(String(1024), nullable=True)
    source_tx: Mapped[Optional[str]] = mapped_column(
        String(128), nullable=True, index=True
    )
    source_ts: Mapped[Optional[datetime]] = mapped_column(
    ...
    __table_args__ = (
        ...
        UniqueConstraint(
            "sim_user", "leader_address", "source_tx", name="uq_sim_trades_source_tx"
        ),
    )
